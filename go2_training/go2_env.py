...
class UnitreeGo2Env(gym.Env):
    ...
    def step(self, action):
        ...
        # Reward shaping
        posture_penalty = 2.3 * (rpy[0] ** 2 + rpy[1] ** 2) # Penalize tilt/encourage staying upright (roll, pitch)
        height_penalty = 13.5 * (z_height - target_height) ** 2 # Encourage maintaining target height
        torque_effort = np.sum(np.square(self.data.ctrl)) # Penalize excessive actuator effort
        alive_bonus = 2.3 # Small constant reward to encourage survival

        # Reward function
        reward = 17 * forward_velocity - height_penalty - posture_penalty - 0.01 * torque_effort + alive_bonus
